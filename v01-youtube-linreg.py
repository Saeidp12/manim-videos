...
class LinReg(Scene): 
    def construct(self):
        ...
        self.wait(3)
        text2 = Tex("تئوری و فلسفه‌ی پشت رگرسیون خطی چیه؟", tex_template = TexTemplateLibrary.persian, font_size=45, color='#FCA311').shift(UP*2.2).shift(RIGHT)
        text1 = Tex("رگرسیون خطی چطور کار می‌کنه؟", tex_template = TexTemplateLibrary.persian, font_size=45, color='#FCA311').next_to(text2,UP).align_to(text2, direction=RIGHT)
        # self.add(text2)
        ...
        text3 = Tex(r"\begin{flushright}در آمار و یادگیری ماشین ما مدل رو\\ بر اساس مشاهدات داده‌هامون می‌سازیم.\end{flushright}",
            font_size=35, color='#FFFFFF', tex_template = TexTemplateLibrary.persian).next_to(dots_group).shift(UP*3)
        ...
        text_estim = Tex(r"\begin{flushright}بهترین راه اینه که با توجه به نمونه\\یک برآورد مناسب از مدل واقعی داشته باشیم.\end{flushright}",
            font_size=35, color='#FFFFFF', tex_template = TexTemplateLibrary.persian).next_to(text5, DOWN*2).shift(RIGHT)
        ...
        text_ols = Tex(r"\begin{flushright}یک راه مناسب، پیدا کردن مدلیه که\\کمترین توان دوم خطای ممکن رو داره.\end{flushright}",
            font_size=35, color='#FFFFFF', tex_template = TexTemplateLibrary.persian).next_to(text_estim, DOWN*1.5).align_to(text_estim, RIGHT)
        ...
        text_mod_list = Tex(r"\item رگرسیون خطی\\\item رگرسیون چندجمله‌ای\\\vdots", 
            font_size=35, color='#FFFFFF', tex_template = TexTemplateLibrary.persian, tex_environment='itemize').shift(RIGHT*4).shift(UP*2)
        ...
        self.wait(5)
        self.play(FadeOut(func), FadeOut(elipse1), FadeOut(text_mod_list))
        
        # Second Scene
        text_this_vid_1 = Tex(r'حالا میتونیم وارد جزئیات رگرسیون خطی',
            font_size=font_size_text, color='#FFFFFF', tex_template = TexTemplateLibrary.persian).shift(RIGHT*2).shift(UP*3).to_edge(RIGHT, buff=1.5)
        text_this_vid_2 = Tex(r'و کمترین توان دوم خطا بشیم.',
            font_size=font_size_text, color='#FFFFFF', tex_template = TexTemplateLibrary.persian).next_to(text_this_vid_1, DOWN).align_to(text_this_vid_1, RIGHT)
        ...
        self.play(Transform(dots_group3, dots_group4))
        self.wait(4)
        ...
        self.play(random_dot.animate.shift(RIGHT*7).shift(UP))
        self.wait(1)
        ...
        # absolute value
        absolute_lines = MathTex(r'\left| \hspace{62 pt} \right|',
            font_size=font_size_math+5).set_color(YELLOW).move_to(text_errors_all.get_center()).shift(RIGHT*1.23).shift(UP*0.05)
        self.play(Write(absolute_lines))
        self.wait(1)

        # Squared Value
        Squared = MathTex(r'(\hspace{62 pt})^2',
            font_size=font_size_math+5).set_color(YELLOW).move_to(text_errors_all.get_center()).shift(UP*0.05).shift(RIGHT*1.35)
        ...
        all_objs.add(text_errors_all[0][0:8], text_distance_i, text_obs_i, model_1d, 
            ...
            text_this_vid_1, text_this_vid_2, dots_group3)
        

        self.play(FadeOut(all_objs))
        self.play(mini_group.animate.shift(LEFT*7.5).shift(UP*3.5))

        # beta_0 derivative
        text_derivative_1 = Tex(r'حالا باید پارامترهایی که این',
            font_size=font_size_text, color='#FFFFFF', tex_template = TexTemplateLibrary.persian).next_to(mini_group, RIGHT*9).shift(UP*0.7)
        text_derivative_2 = Tex(r'عبارت رو کمینه می‌کنند، پیدا کنیم.',
        ...
        text_zero = Tex('برای هر پارامتر مشتق باید برابر',
            font_size=font_size_text, color='#FFFFFF', tex_template = TexTemplateLibrary.persian).next_to(text_derivative_2, DOWN*2).align_to(text_derivative_2, RIGHT)
        ...
        text_multiple_vars = Tex('اگر بیشتر از یک متغیر مستقل داشته باشیم',
            font_size=font_size_text, color='#FFFFFF', tex_template = TexTemplateLibrary.persian).next_to(text_zero_2, DOWN*2).align_to(text_zero, RIGHT)
        ...
        text_linalg = Tex('در اینجا باید از خواص جبر خطی و ماتریس‌ها',
            font_size=font_size_text, color='#FFFFFF', tex_template = TexTemplateLibrary.persian).next_to(text_multiple_vars_2, DOWN*2).align_to(text_multiple_vars_2, RIGHT)
        ...
        text_obs_all = MathTex(r'''y_1 &= \beta_0 + \beta_1 x_{11} + \cdots + \beta_m x_{1m}\\
            ...
            \vdots & \\
            y_n &= \beta_0 + \beta_1 x_{n1} + \cdots + \beta_m x_{nm}''', font_size=font_size_matrices-3).shift(LEFT*4).shift(UP*2)
        ...
        y_mat = MathTex(r'''
            ...
            & \vdots\\
            & y_{n}
        ''', font_size=font_size_matrices-3)
        ...
        equals_2 = MathTex(r' = ', font_size=32).next_to(final_mat_bracket, RIGHT)
        ...
        yxbeta = VGroup()
        y = MathTex(r'y =', font_size=font_size_math-3).next_to(y_mat, DOWN*2).align_to(y_mat[1], LEFT)
        text_then = MathTex(r'\Rightarrow', font_size=font_size_math-3).next_to(y, LEFT)
        X = MathTex(r'X',font_size=font_size_math-3).next_to(y, RIGHT).shift(UP*0.05)
        beta = MathTex(r'\beta',font_size=font_size_math-3).next_to(X, RIGHT*0.5).shift(DOWN*0.05)
        yxbeta.add(y, X, beta)
        ...
        self.play(FadeOut(all_objs_2), yxbeta.animate.shift(UP*0.5))
        self.wait(1)
        ...
        sse_mat = MathTex(r'(y - X\beta)^{\prime}(y - X\beta)', font_size=font_size_math-5).next_to(yxbeta, DOWN).align_to(mini_group[5], LEFT)
        self.play(Write(sse_mat))
        self.wait(3.5)

        # open up the matrix form
        sse_mat_open = MathTex(r' = y^{\prime}y - y^{\prime}X\beta - \beta^{\prime}X^{\prime}y + \beta^{\prime}X^{\prime}X\beta', font_size=font_size_math-5).next_to(sse_mat, RIGHT)
        self.play(Write(sse_mat_open))
        self.wait(5)

        # beta derivation
        beta_derivative_text_1 = Tex('اینجا باید مشتق نسبت به بردار پارامترها رو',
            font_size=font_size_text-5, color='#FFFFFF', tex_template = TexTemplateLibrary.persian).next_to(mini_group, RIGHT*2).to_edge(RIGHT, buff=1).shift(UP)
        beta_derivative_text_2 = Tex('به جای هر یک از پارامترها پیدا کنیم.',
            font_size=font_size_text-5, color='#FFFFFF', tex_template = TexTemplateLibrary.persian).next_to(beta_derivative_text_1, DOWN).align_to(beta_derivative_text_1,RIGHT)
        ...
        dbeta_note_1 = Tex('برای انجام این کار باید دو مورد از خواص', 
            font_size=font_size_text-5, color='#FFFFFF', tex_template = TexTemplateLibrary.persian).next_to(beta_derivative_text_2, DOWN*1.5).align_to(beta_derivative_text_1, RIGHT)
        dbeta_note_2 = Tex('مشتق بردارها را به خاطر داشته باشیم:',
            font_size=font_size_text-5, color='#FFFFFF', tex_template = TexTemplateLibrary.persian).next_to(dbeta_note_1, DOWN).align_to(beta_derivative_text_2, RIGHT)
        ...
        prop1 = MathTex(r'\frac{\partial}{\partial a} a^{\prime}b = \frac{\partial}{\partial a} b^{\prime}a = b',
            font_size=font_size_math-5).next_to(dbeta_note_2, DOWN*2)
        prop2 = MathTex(r'\frac{\partial}{\partial a} a^{\prime}Ba = 2Ba',
            font_size=font_size_math-5).next_to(prop1, DOWN).align_to(prop1, LEFT)
        ...
        self.play(FadeOut(all_objs_3, mini_group, yxbeta), 
            dbeta_group.animate.shift(UP*5.5).to_edge(LEFT, buff=1), 
            surrect_prop_group.animate.shift(UP*3).to_edge(RIGHT, buff=1).scale(0.7))
        self.wait()
        ...
        d_multiple_2 = d_multiple.copy().move_to(d_eq_simp_right.get_center()).align_to(d_eq_simp_right, LEFT).shift(RIGHT*2.9)
        ...
        self.wait(2)
        # channel_insta = Text('mlwithsaeid', font='Lato', font_size=60).shift(UP*0.5).shift(LEFT*0.5)
        # channel_youtube = Text('SaeidPourmand', font='Lato', font_size=60).next_to(channel_insta,DOWN).align_to(channel_insta, LEFT)
        # instagram = ImageMobject('E:/Youtube 2/instagram.png').next_to(channel_insta, RIGHT)
        # youtube = ImageMobject('E:/Youtube 2/youtube.png').next_to(channel_youtube, RIGHT).scale(0.4)
        # channel_vg = VGroup(channel_youtube, channel_insta)
        # self.play(Transform(fin_group[1], channel_vg), FadeIn(instagram), FadeIn(youtube))
        # self.wait(15)